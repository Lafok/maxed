...
public class ChatController {
    ...
    public ResponseEntity<ChatResponse> createDirectChat(@RequestBody CreateDirectChatRequest request) {
        User currentUser = principalProvider.getAuthenticatedUserEntity();
        ChatResponse chatResponse = chatService.createDirectChat(request, currentUser);
    ...
    public ResponseEntity<List<ChatResponse>> getChatsForCurrentUser() {
        User currentUser = principalProvider.getAuthenticatedUserEntity();
        return ResponseEntity.ok(chatService.getChatsForCurrentUser(currentUser));
    }
    ...
    public ResponseEntity<Page<MessageResponse>> getMessagesForChat(@PathVariable Long chatId, @ParameterObject Pageable pageable) {
        User currentUser = principalProvider.getAuthenticatedUserEntity();
        return ResponseEntity.ok(chatService.getMessagesForChat(chatId, pageable, currentUser));
    }
}
